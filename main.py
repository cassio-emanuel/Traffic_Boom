...
from carros import Inimigos
from const import WIN_WIDTH, WIN_HEIGHT

pygame.init()
def mostrar_menu():
    titulo_fonte = pygame.font.SysFont("Arial", 60, bold=True)
    opcao_fonte = pygame.font.SysFont("Arial", 36)
    ...
    while True:
        ...
        # Título do jogo
        titulo = titulo_fonte.render("Traffic Boom", True, (255, 255, 0))
        window.blit(titulo, (WIN_WIDTH // 2 - titulo.get_width() // 2, 100))

        # Opções de dificuldade
        for i, texto in enumerate(opcoes):
            cor = (255, 0, 0) if i == selecionado else (255, 255, 255)
...
while run:
    ...
    # Verifica colisão do jogador com minas
    for mina in minas_ativas:
    ...
    if game_over:
        ...
        keys = pygame.key.get_pressed()
        if keys[pygame.K_RETURN]:  # se apertar ENTER, reinicia
            # Resetando o jogo
            game_over = False
            score = 0
            tempo_inicio = pygame.time.get_ticks()
